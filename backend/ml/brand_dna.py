...
Used in features.py as a last-resort fallback when a perfume has no note pyramid
and no accord tags — lets the feature vector reflect the brand's typical chemistry
profile (at 30 % weight) rather than staying at neutral 5.0 defaults.
...
# 10 fields that match _CHEM_FIELDS + longevity_class in features.py
_PRIOR_FIELDS = [
...
def initialize_brand_dna(perfumes: list[dict]) -> None:
    """
    Optionally pre-populate the brand cache from an external list of perfume dicts
